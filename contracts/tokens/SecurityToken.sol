...
contract SecurityToken is ERC20, ReentrancyGuard, SecurityTokenStorage, IERC1594, IERC1643, IERC1644, IERC1410, Proxy {
    ...
    function _isValidOperator(address _from, address _operator, bytes32 _partition) internal view {
        _isAuthorised(
            allowance(_from, _operator) == type(uint).max || partitionApprovals[_from][_partition][_operator]
        );
    }
    ...
    */
    ...
    function authorizeOperator(address _operator) public {
        _approve(msg.sender, _operator, type(uint).max);
     ...
     * @return toPartition partition to which the transferred tokens were allocated for the _to address
     */
