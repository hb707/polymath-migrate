...
abstract contract UpgradableModuleFactory is ModuleFactory {
    ...
    function upgrade(address _module) external {
        ...
        // Change address to payable address
        address payable _modulePayable = payable(_module);
        OwnedUpgradeabilityProxy(_modulePayable).upgradeToAndCall(logicContracts[newVersion].version, logicContracts[newVersion].logicContract, logicContracts[newVersion].upgradeData);
